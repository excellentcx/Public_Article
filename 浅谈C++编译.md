...
目标文件是从源代码文件产生程序文件这一过程的中间产物。它是编译器编译源代码后生成的文件。（Windows下面为.obj文件；Linux下面为.o文件）从结构上讲，它是已经编译后的可执行文件格式，**只是还没有经过链接的过程**，其中可能有些符号或有些地址还没有被调整，目标文件本身就是按照可执行文件格式存储的。
...
第四：静态库对程序的更新、部署和发布页会带来麻烦。如果静态库liba.lib更新了，所以使用它的应用程序都需要重新编译、发布给用户。
